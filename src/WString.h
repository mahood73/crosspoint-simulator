...
public:
  ...
  void replace(const char *find, const char *replace) {
    ...
    const std::string replacement(replace ? replace : "");
    size_t pos = 0;
    while ((pos = s.find(needle, pos)) != std::string::npos) {
      s.replace(pos, needle.length(), replacement);
      pos += replacement.length();
    }
  }
  void replace(unsigned int index, unsigned int count, const char *replace) {
    if (index > s.length())
      return;
    s.replace(index, count, replace ? replace : "");
  }
